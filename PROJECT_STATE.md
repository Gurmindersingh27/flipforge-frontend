...
Added `tests/productionSmoke.mjs` and `.github/workflows/release-verification.yml` to make the public check repeatable. The script uses only native Node APIs and existing build dependencies; it writes an evidence JSON even on failure. Run from a clean checkout of the intended frontend release after npm ci. The workflow checks out production source separately from its checker, runs for changes to the checker and on manual dispatch, and uploads results. It performs three stateless analysis requests, never a saved-deal write. It does not test signed-in browser behavior. New workflow publication/CI is pending at this local record.

## 2026-09-11 — Scope evidence comparison improvement

The previous comparison noticed item changes but displayed only category, line price and latest provenance. A cost-neutral change to exclusions/specification could be invisible in the summary. Added a structured diff of description, quantity/unit/rate, basis, contractor, date and exact notes; shows previous/current passages, changed budget notes and optional monthly rent. Reordering item/object keys does not invent changes. Add/remove labels refer to itemization, not verified scope coverage.

Files: src/lib/revisionDiff.ts, src/components/RevisionComparison.tsx, tests/revisionDiff.test.ts, tests/browserFlow.mjs, PROJECT_STATE.md. No shared types, engine, saved records, snapshots, API or dependencies changed. Browser CI remains pinned to the already deployed backend contract; the parallel backend experiment changes no runtime behavior.

Local verification: 37 frontend tests PASS (29 existing + 8 meaningful evidence regressions), production build PASS (118 modules), same 10 pre-existing lint errors; no new lint errors. Extended browser regression saves a cost-neutral third revision, asserts both exclusion passages, unchanged modeled economics, unchanged prior record and mobile layout. That new browser run is pending publication/CI at this entry.

Frontend PR #62 contains the public checker, closeout record and this improvement; it is a draft for review, not merged or deployed. Full Quote Review + Missing Scope v1 remains unbuilt in the production UI. The backend branch contains an explicitly synthetic manual-review CLI experiment; real quote-pair validation and structured review persistence remain outstanding.
## Bid Impact v1 — implementation on a stacked draft branch, 2026-09-11

- Base: frontend PR #62 at `35e5014a52811d5490363e34ee08c94098dad890`. Main is still `0e56e784efbc41cc840c6bd1e1e18a6212eca92b`; no new merge or production deployment is claimed.
- Branch: `codex/bid-impact-v1`, intended to be reviewed against `codex/release-closeout-20260911`. Review/merge #62 before retargeting this work to main. Existing CI is enabled for the stacked PR's base branch.
- Quote entry: apply contractor/date only to selected lines. Converting allowances and replacing existing quote provenance require separate explicit confirmations. Amounts, IDs, notes and unselected sources remain intact. Categories stay editable, with suggestions matching the existing budget builder.
- Comparison: load the owner's quoted sibling revisions through existing `getDeals`. Compare exactly two options using their saved results. Both require a common baseline/owner, valid scope totals/provenance and matching recorded property/non-rehab assumptions, including the baseline. Missing historical inputs block comparison instead of receiving silent defaults.
- Cost display separates new/changed quoted lines, new/changed allowances, unchanged baseline lines, subtotal and contingency. Raw line source/date/descriptions/exclusions and budget notes remain visible. Missing category itemization is explicitly unknown coverage, not a zero price or proof of exclusion. Category synonyms are not inferred.
- Continue opens a selected saved bid through the existing Resume flow. Saving produces a child of that bid; it does not edit the baseline or other bids.
...
## Saved-deal read recovery - September 24, 2026 continuation

- Starting state freshly verified: frontend main `2adf36038856444d57225abf914a8b1fcecb4219` (#66), backend main `14a7dcedbe4ef2c914ea786122bdff88a2a5e35f` (#21). Render deployment `dep-daqunvqvcj2c739rm3dg` reports live. The September 24 master handoff supersedes earlier pending-release notes above.
- Public checks in this continuation: health HTTP 200 in approximately 5.0 and 5.5 seconds; stateless manual baseline analysis HTTP 200 in 4.8 seconds, correct production CORS, BUY / $34,900 profit / $155,600 max offer. These are individual observations from the execution environment, not measured browser cold starts or database idle/wake tests.
- Problem found in source: saved reads shared a 60-second fetch timeout, loading copy never explained a longer wait, and list/detail failures offered no retry. Render documents roughly one minute to spin up a free service (https://render.com/docs/free); this is a readiness risk, not proof a production user timed out.
- Scope: only saved GET requests receive a 120-second deadline covering headers and body. After eight seconds, list/detail loading explains the wait without asserting a cause. A failed load offers a user-triggered retry that obtains a fresh auth token. There is no automatic retry or change to save requests, analysis, contracts, calculation math, ownership, snapshots, providers, or dependencies.
- Local validation before publication: 67 unit tests pass; production build passes with the existing CSS import warning; targeted ESLint for the three changed source files passes. This does not claim the full lint baseline is clean. The browser regression adds delayed success, header/body timeout, authenticated user retry, 403 handling, mobile overflow, and unchanged-record assertions. New browser checks require CI at the published head because local Chrome is unavailable. Browser CI retains its existing backend #20 pin and isolated SQLite/auth fixtures; it does not test live Neon.
- Status: implemented locally for independent PR review; not merged, deployed or user-tested. Backup retention and restore remain unverified: no connected Neon settings access or production recovery drill was available. Signed-in idle/revision/bid checks remain user-deferred. No hosting upgrade, paid service, outreach, billing, or customer traction was created.

## Analyzer and save cold-start recovery - September 25, 2026

- Starting release: frontend PR #67 was independently approved by Claude at `d64dd4ac0af2622980125c156bc58cf8d731973f`, then merged as `7499658632e09e8ca99565462167061bfada55ec`. All 13 public production checks passed, including byte-for-byte JS/CSS matching; release evidence is recorded on #67. Backend remains #21 at `14a7dcedbe4ef2c914ea786122bdff88a2a5e35f`. These checks did not exercise signed-in Neon recovery.
- Remaining problem: manual analysis, draft finalization, URL drafting and save requests still had 60-second header-only deadlines and no delayed-request explanation. Merely opening the frontend does not prove that its backend is awake.
- This branch extends only those four requests to a 120-second deadline covering response bodies. There are no automatic retries. Each associated UI explains a long wait after eight seconds and clears the explanation when the request finishes or the UI unmounts. Analysis errors retain inputs for a user-triggered retry; both existing 422 missing-field response shapes remain supported.
- Save attempts lock before session-token retrieval to prevent rapid duplicate submissions. A timeout, network failure, server error or invalid success response is treated as an unconfirmed save, with a link to Saved Deals and no success claim or repeat-save button for that memo. A received 4xx rejection remains distinct. Starting a different analysis while token retrieval is pending prevents the obsolete save from being sent; late failures cannot overwrite a different memo's save status. No server idempotency or cross-tab duplicate prevention is claimed.
- Files: `src/lib/api.ts`, `src/App.tsx`, `tests/browserFlow.mjs`, `PROJECT_STATE.md`. No backend, schemas, calculation math, frozen snapshots, owner checks, provider paths, dependencies, hosting plans or paid calls changed.
